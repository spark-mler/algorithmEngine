...
class MultiSVM (classModels: Array[SVMModel]) extends ClassificationModel with Serializable{
...
object SVMMultiClassOVAWithSGD {
  ...
  def train(
             ...
             stepSize: Double,
             regParam: Double,
             miniBatchFraction: Double): MultiSVM = {
  ...
  /**
    ...
    */
  def train(input: RDD[LabeledPoint], numIterations: Int, stepSize: Double, regParam: Double): MultiSVM =
  ...
  /**
    ...
    * @return a SVMModel which has the weights and offset from training.
    */
  def train(input: RDD[LabeledPoint], numIterations: Int): MultiSVM = train(input, numIterations, 1.0, 0.01, 1.0)
